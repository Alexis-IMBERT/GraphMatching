...
def get_assignment_from_labelling(
    list_graphs: list, labelling_attribute_name, excluded_labels: bool = False
):
    ...
    all_graphs_labels = get_labelling_from_attribute(
        list_graphs, labelling_attribute_name
    )
    ...
    if excluded_labels is not None:
        for ind_node, label in zip(
            range(tot_nb_nodes), row_index_list_all_graphs_labels
        ):
    ...
    dummy_mask = [
        list(nx.get_node_attributes(graph, "is_dummy").values())
        for graph in list_graphs
    ]
    dummy_mask = sum(dummy_mask, [])
    dummy_indexes = [i for i in range(len(dummy_mask)) if dummy_mask[i]]

    X_w_dummy = insert_at(
        X, (sum(sizes_dummy), sum(sizes_dummy)), (dummy_indexes, dummy_indexes)
    )
