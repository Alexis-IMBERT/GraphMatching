...
if __name__ == "__main__":
    file_mesh = os.path.join(
        project_path,
        "data/Oasis_all_subjects_white/FS_OASIS/OAS1_0439/surf/lh.white.gii",
    )
    ...
    for n in graph.nodes:
        ...
        matching_labels_tex[tex_basins.darray[0] == basin_tex_label] = (
            lab_matching_value
        )

    # Plot the mesh with the matching labels texture
    vb_sc2 = gv.visbrain_plot(
        mesh,
        tex=matching_labels_tex,
        cmap="jet",
    ...
    s_obj2, c_obj2, node_cb_obj2 = gv.show_graph(
        graph,
        nodes_coords,
        node_color_attribute="labelling_mSync",
    ...
    vb_sc2.add_to_subplot(
        node_cb_obj2, row=visb_sc_shape[0] - 1, col=visb_sc_shape[0] + 0, width_max=200
    )
