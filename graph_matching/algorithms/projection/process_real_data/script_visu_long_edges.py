...
if __name__ == "__main__":
    ...
    vb_sc = gv.visbrain_plot(
        template_mesh,
        caption="Visu on template_mesh mesh of edges longer than "
        + str(geo_length_threshold),
    )
    ...
    vb_sc2 = gv.visbrain_plot(
        sphere_mesh,
        caption="Visu on sphere of edges longer than " + str(geo_length_threshold),
    )

    for graph in list_graphs:
        gp.remove_dummy_nodes(graph)
        nodes_coords = gp.graph_nodes_to_coords(
            graph, "ico100_7_vertex_index", sphere_mesh
        )
        # manage nodes
        s_obj, nodes_cb_obj = gv.graph_nodes_to_sources(graph, nodes_coords)
        # manage edges
