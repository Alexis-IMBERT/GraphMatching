...
def save_labelled_graphs(list_graphs, path_to_save):
    for i in range(len(list_graphs)):
        Visualisation(list_graphs[i]).save_as_pickle(
            os.path.join(path_to_save, "graph_{:05d}".format(i))
        )
...
if __name__ == "__main__":
    ...
    path_to_save_labelled_graphs = os.path.join(
    ...
    for ind, method in enumerate(methods):
        path_to_X = os.path.join(
            project_path,
            "data/Oasis_original_new_with_dummy/X_" + method + reg_or_unreg + ".mat",
        )
        print("----------------------------")
        print(method)
    ...
    for ind, method in enumerate(methods):
        ...
        transfered_labels_all_graphs = gca.get_labelling_from_attribute(
            list_graphs, labelling_attribute_name=label_attribute
        )

        a_transfered_labels_all_graphs = np.array(
            gca.concatenate_labels(transfered_labels_all_graphs)
        )
        print(np.unique(a_transfered_labels_all_graphs))
        ax[0, ind].hist(
            a_transfered_labels_all_graphs, density=dens, bins=nb_bins
