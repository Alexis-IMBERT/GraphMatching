...
if __name__ == "__main__":
    ...
    methods = [
        ...
        "media",
        "neuroimage",
    ]  # ,'kmeans_70_real_data_dummy','kmeans_90_real_data_dummy','kmeans_110_real_data_dummy']
    ...
    for ind, method in enumerate(methods):
        ...
        pickle_out = open(
            os.path.join(
                path_to_silhouette, "labelling_" + method + "_silhouette.gpickle"
            ),
            "rb",
        )
        silhouette_dict = p.load(pickle_out)
        pickle_out.close()
        clust_silhouette, clust_nb_nodes = gca.get_silhouette_per_cluster(
            silhouette_dict
        )
        ...
        ax[0, ind].hist(
            clust_silhouette, density=dens, bins=nb_bins
        ...
        ax[1, ind].hist(
            clust_nb_nodes_perc, density=dens, bins=nb_bins
        )  # density=False would make counts
