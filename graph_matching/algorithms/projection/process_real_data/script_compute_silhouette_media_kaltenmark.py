...
if __name__ == "__main__":
    ...
    path_to_silhouette = os.path.join(
        project_path, "data/Oasis_original_new_with_dummy/silhouette"
    )
    reg_or_unreg = "_unreg"  # Modify this based on your requirement

    # Define paths to assignment matrices
    path_to_mALS = os.path.join(
        project_path, f"data/Oasis_original_new_with_dummy/X_mALS{reg_or_unreg}.mat"
    )
    path_to_mSync = os.path.join(
        project_path, f"data/Oasis_original_new_with_dummy/X_mSync{reg_or_unreg}.mat"
    )
    path_to_CAO = os.path.join(
        project_path, "data/Oasis_original_new_with_dummy/X_CAO.mat"
    )
    path_to_kerGM = os.path.join(
        project_path,
        f"data/Oasis_original_new_with_dummy/X_pairwise_kergm{reg_or_unreg}.mat",
    )
    ...
    gca.get_labelling_from_assignment(
        list_graphs, X_kerGM, largest_ind, mesh, label_attribute
    )
    ...
    silhouette_path = os.path.join(
        path_to_silhouette, f"{label_attribute}_silhouette.gpickle"
    )
