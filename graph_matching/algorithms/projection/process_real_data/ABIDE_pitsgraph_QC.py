...
if __name__ == "__main__":
    ...
    graph_files_list = [
        ...
        if hemi in fil and fil.endswith(".gpickle")
    ]
    ...
    subjects_mean_depth = [
        np.mean(pg.graph_nodes_attribute(g, "depth")) for g in pitgraphs_list
    ]
    subjects_nb_pits = [len(g) for g in pitgraphs_list]

    # Load phenotype data
    subjects_info = os.path.join(
        project_path, "data/ABIDE/Phenotypic_V1_0b_traitements_visual_check_GA.xls"
    )
    ...
    inds_keep = [
        subjects_list_table.index(s) for s in subjects_list if s in subjects_list_table
    ]
    ...
    print(
        f"Number of matching subjects: {len(set(subjects_list_check).intersection(set(subjects_list)))}"
    )
    ...
    axes[1, 0].hist(
        [asd_nb_pits, ctrl_nb_pits],
        label=[f"{np.sum(asd_inds)} ASD", f"{np.sum(ctrl_inds)} CTRL"],
        density=True,
    )
    axes[1, 0].legend()
    axes[1, 0].set_title("Number of pits by diagnosis")
    axes[1, 1].hist(
        [asd_mean_depth, ctrl_mean_depth],
        label=[f"{np.sum(asd_inds)} ASD", f"{np.sum(ctrl_inds)} CTRL"],
        density=True,
    )
    axes[1, 1].legend()
