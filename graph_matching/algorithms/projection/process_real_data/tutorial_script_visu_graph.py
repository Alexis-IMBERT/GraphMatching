...
if __name__ == "__main__":
    file_template_mesh = os.path.join(
        project_path, "data/template_mesh/lh.OASIS_testGrp_average_inflated.gii"
    )
    file_mesh = os.path.join(
        project_path, "data/example_individual_OASIS_0061/rh.white.gii"
    )
    file_basins = os.path.join(
        project_path,
        "data/example_individual_OASIS_0061/alpha0.03_an0_dn20_r1.5_R_area50FilteredTexture.gii",
    )
    file_graph = os.path.join(
        project_path, "data/example_individual_OASIS_0061/OAS1_0061_rh_pitgraph.gpickle"
    )
    ...
    vb_sc = gv.visbrain_plot(
        mesh,
        tex=tex_basins.darray[2],
        cmap="tab20c",
    ...
    s_obj2, c_obj2, node_cb_obj2 = gv.show_graph(
        graph,
        nodes_coords,
        node_color_attribute="depth",
    ...
    vb_sc2.add_to_subplot(
        node_cb_obj2, row=visb_sc_shape[0] - 1, col=visb_sc_shape[1] + 1, width_max=200
    )
    ...
    nodes_coords = gp.graph_nodes_to_coords(
        graph, "ico100_7_vertex_index", template_mesh
    )
    # 3 eventually compute a mask for masking some part of the graph
    mask_slice_coord = -15
    nodes_mask = nodes_coords[:, 2] > mask_slice_coord
    # 4 create the objects for visualization of the graph and add these to the figure
    s_obj3, c_obj3, node_cb_obj3 = gv.show_graph(
        graph,
        nodes_coords,
        node_color_attribute="basin_thickness",
        edge_color_attribute="geodesic_distance",
        nodes_mask=nodes_mask,
    )
    ...
    vb_sc3.add_to_subplot(
        node_cb_obj3, row=visb_sc_shape[0] - 1, col=visb_sc_shape[1] + 1, width_max=200
    )
