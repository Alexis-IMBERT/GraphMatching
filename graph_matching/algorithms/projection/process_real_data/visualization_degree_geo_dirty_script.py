...
def remove_dummy_nodes(graph):
    G = graph.copy()
    to_remove = [p for p, d in G.nodes(data=True) if d.get("is_dummy", False)]
...
avg_nb_degree_all_graph = [
    [np.mean(lst) for lst in degree_dict.values()]
...
def geo_dist_of_neighb(G):
    nb_dist_dict = {
        node: [
            G.get_edge_data(node, nb)["geodesic_distance"] for nb in G.neighbors(node)
        ]
        for node in G
    }
    return nb_dist_dict
...
avg_nb_dist_all_graph = [
    [np.mean(lst) for lst in dist_dict.values()] for dist_dict in all_graph_nb_distance
]
