...
def create_clusters_lists_with_label_gender(
    list_graphs, gender_corresp, label_attribute="label_dbscan"
):
    result_dict = {}
    label_depths = {}
    label_gender = {}
...
def calculate_tstats_and_pvalues(corresp, method="labelling_mALS"):
    result_dict, label_depths, label_gender = create_clusters_lists_with_label_gender(
        labeled_graphs, corresp, method
    )
...
if __name__ == "__main__":
    ...
    path_ro_correspondence = os.path.join(
        project_path, "data/gender_correspondence.pickle"
    )

    labeled_graphs = gp.load_graphs_in_list(path_to_labelled_graphs)
    gender_corresp = np.array(p.load(open(path_ro_correspondence, "rb")))[
        :, 2
    ]  # gender corresp list
    ...
    for ind, method in enumerate(methods):
        ...
        cluster_dict, label_depths, label_gender = (
            create_clusters_lists_with_label_gender(
                labeled_graphs, gender_corresp, label_attribute=label_attribute
            )
        )
        centroid_dict = gca.get_centroid_clusters(
            labeled_graphs, cluster_dict, coords_attribute="sphere_3dcoords"
        )
        t_stats_method = calculate_tstats_and_pvalues(gender_corresp, label_attribute)
        t_stat, centroid_dict = drop_nan_get_tstats(
            t_stats_method, centroid_dict
        )  # drop nan value obtained for labels that contain only 1 group from t-stats.
