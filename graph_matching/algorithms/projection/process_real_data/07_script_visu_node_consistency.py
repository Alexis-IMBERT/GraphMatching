...
if __name__ == "__main__":
    ...
    path_to_consistency = os.path.join(
        project_path, "data/Oasis_original_new_with_dummy/consistency"
    )
    path_to_figs = os.path.join(
        project_path, "data/Oasis_original_new_with_dummy/figures"
    )
    ...
    pickle_in_path = os.path.join(
        path_to_consistency, "nodeCstPerGraph_" + method + reg_or_unreg + ".pck"
    )
    ...
    for ind_g, g in enumerate(list_graphs):
        ...
        s_obj, nodes_cb_obj = gv.graph_nodes_to_sources(
            nodes_coords,
            node_data=data_node_cstr[data_mask],
            nodes_size=None,
            nodes_mask=None,
            c_map=c_map,
            symbol="disc",
