...
if __name__ == "__main__":
    ...
    path_to_match_mat = os.path.join(
        project_path, "data/_obsolete_OASIS_full_batch/Hippi_res_real_mat.npy"
    )
    ...
    g_l = p.load(
        open(
            os.path.join(
                project_path,
                "data/_obsolete_OASIS_full_batch/modified_graphs/graph_"
                + str(largest_ind)
                + ".gpickle",
            ),
            "rb",
    ...
    for j in range(len(list_graphs)):
        ...
        nodes_to_remove = np.where(not np.array(nodes_to_remove))
    ...
    for matching_matrix in X:
        ...
        for i in range(nb_graphs):
            ...
            nodes_coords = gp.graph_nodes_to_coords(
                g, "ico100_7_vertex_index", reg_mesh
            )
            s_obj, nodes_cb_obj = gv.graph_nodes_to_sources(
                nodes_coords,
                node_data=transfered_labels,
                nodes_mask=None,
                c_map="nipy_spectral",
            )
