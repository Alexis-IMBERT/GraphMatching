...
class TestManOpt(TestCase):
    def test_get_top_left(self):
        result = np.array([[2, 0, 0], [0, 2, 0], [0, 0, 2]])
        self.assertTrue(
            np.allclose(concatenation.get_top_left(np.ones((3, 2))), result)
        )

    def test_get_bottom_right(self):
        result = np.array([[3, 0], [0, 3]])
        self.assertTrue(
            np.allclose(concatenation.get_bottom_right(np.ones((3, 2))), result)
        )
